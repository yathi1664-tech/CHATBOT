...
function pushBodyForMessage(message) {
  if (message.kind === "text") return String(message.text || "New message").slice(0, 180);
  if (message.kind === "attachment") return `Sent ${String(message.file?.name || "an attachment").slice(0, 130)}`;
  if (message.kind === "sticker") return `Sent a sticker ${message.sticker || ""}`;
  return "New message";
}
...
async function sendSystemPush(message) {
  ...
  const payload = JSON.stringify({
    title: message.name || "Private Chat",
    body: pushBodyForMessage(message),
    url: "/",
