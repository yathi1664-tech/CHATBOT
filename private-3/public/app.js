...
notificationToggle.addEventListener('change',async()=>{
  if(notificationToggle.checked){
    if(!('Notification' in window)){
      notificationsEnabled=false;notificationToggle.checked=false;updateNotificationLabel();showStatus('Browser notifications are not supported on this device.',true);return;
    }
    let permission=Notification.permission;
    if(permission==='default') permission=await Notification.requestPermission();
    if(permission!=='granted'){
      notificationsEnabled=false;notificationToggle.checked=false;localStorage.setItem('privateChatNotifications','off');updateNotificationLabel();showStatus('Notification permission was not allowed.',true);return;
    }
    notificationsEnabled=true;
  }else notificationsEnabled=false;
  localStorage.setItem('privateChatNotifications',notificationsEnabled?'on':'off');
  updateNotificationLabel();
  showNotificationToast(notificationsEnabled?'New message notifications turned on.':'New message notifications turned off.');
});
...
function notifyIncomingMessage(message){
  ...
  const sender=message.name||'Private Chat';
  let body='New message';
  if(message.kind==='text') body=message.text||'New message';
  else if(message.kind==='attachment') body=`Sent ${message.file?.name||'an attachment'}`;
  else if(message.kind==='sticker') body=`Sent a sticker ${message.sticker||''}`;
  playNotification(true);
  if('Notification' in window && Notification.permission==='granted' && document.visibilityState!=='visible'){
    try{const n=new Notification(sender,{body:String(body).slice(0,180),tag:`private-chat-${message.id||Date.now()}`,renotify:true});n.onclick=()=>{window.focus();n.close();};}catch(_){}
  }else{
    showNotificationToast(`${sender}: ${String(body).slice(0,120)}`);
  }
}
