...
const messageMenu=$('messageMenu'), replyMessageBtn=$('replyMessageBtn'), deleteForMeBtn=$('deleteForMeBtn'), deleteForEveryoneBtn=$('deleteForEveryoneBtn');
...
let replyingTo = null, menuMessage = null;
...
function connectSocket(){if(socket?.connected)return;socket=io();socket.on('connect',()=>{socket.emit('set-client-id',clientId);socket.emit('set-name',myName||'Guest');});socket.on('connect_error',()=>{chatScreen.classList.add('hidden');loginScreen.classList.remove('hidden');loginError.textContent='Session expired. Enter the password again.';});
socket.on('presence',({count,users})=>{onlineUsers=users||[];presenceText.textContent=`${count} ${count===1?'member':'members'} online`;});
socket.on('chat-history',history=>{messages.innerHTML='';const visible=(history||[]).filter(m=>!deletedForMe.has(m.id));if(!visible.length)messages.innerHTML='<div class="empty">No messages yet.<br>Start the private conversation.</div>';else visible.forEach(renderMessage);scrollBottom();});
socket.on('owned-message-ids',ids=>{ownedMessageIds=new Set(ids||[]);refreshOwnershipButtons();});
socket.on('message-owned',({messageId})=>{ownedMessageIds.add(messageId);refreshOwnershipButtons();});
socket.on('chat-message',m=>{if(deletedForMe.has(m.id))return;messages.querySelector('.empty')?.remove();renderMessage(m);scrollBottom();playNotification();});
...
function renderMessage(message){const article=document.createElement('article');article.className='message';article.dataset.messageId=message.id;article._messageData=message;
const meta=document.createElement('div');meta.className='message-meta';const name=document.createElement('span');name.className='message-name';name.textContent=message.name;const time=document.createElement('span');time.textContent=new Date(message.time).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});meta.append(name,time);
const actions=document.createElement('div');actions.className='message-actions';const menuBtn=document.createElement('button');menuBtn.type='button';menuBtn.textContent='⋮';menuBtn.setAttribute('aria-label','Message options');menuBtn.onclick=(e)=>{e.stopPropagation();openMessageMenu(article,message,menuBtn);};actions.appendChild(menuBtn);article.append(meta,actions);
...
function openMessageMenu(article,message,button){menuMessage={article,message};deleteForEveryoneBtn.classList.toggle('hidden',!ownedMessageIds.has(message.id));const rect=button.getBoundingClientRect();messageMenu.classList.remove('hidden');const menuRect=messageMenu.getBoundingClientRect();let left=Math.min(rect.right-menuRect.width,window.innerWidth-menuRect.width-8);left=Math.max(8,left);let top=rect.bottom+6;if(top+menuRect.height>window.innerHeight-8)top=Math.max(8,rect.top-menuRect.height-6);messageMenu.style.left=`${left}px`;messageMenu.style.top=`${top}px`;}
function closeMessageMenu(){messageMenu.classList.add('hidden');menuMessage=null;}
function refreshOwnershipButtons(){if(menuMessage)deleteForEveryoneBtn.classList.toggle('hidden',!ownedMessageIds.has(menuMessage.message.id));}
