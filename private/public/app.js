...
socket.on('chat-message',m=>{if(deletedForMe.has(m.id))return;messages.querySelector('.empty')?.remove();renderMessage(m);scrollBottom();playNotification();setTimeout(markMessagesSeen,80);});
...
function renderMessage(message){
  ...
  delivery.textContent=`Sent ${formatTime(message.time)}${message.seenAt?` · Seen ${formatTime(message.seenAt)}`:''}`;
...
function updateSeenStatus(update){
  ...
  if(status){const sent=article._messageData?.time||new Date().toISOString();status.textContent=`Sent ${formatTime(sent)} · Seen ${formatTime(update.seenAt)}`;status.title=`Seen by ${update.seenBy||'the other member'} at ${formatDateTime(update.seenAt)}`;}
}
